...
function tc_patient_deactivate_a_patient()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient('Regression', 'Deactivate_patient', 'M'); 
...
function tc_patient_reactivate_a_patient()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient("Regression", "Activate_patient", "M"); 
    deactivate_patient();
...
function tc_patient_amend_a_patient_to_be_a_manual_self_tester()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient('Regression', 'self_test_patient', 'M'); 
...
function tc_patient_suspend_a_patient()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient('Regression', 'suspend_patient', 'M'); 
...
function tc_suspend_a_patient_user_unable_to_select_a_date_more_than_6_months_in_the_future()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient('Regression', 'suspend_limit', 'M'); 
...
function tc_patient_unsuspend_a_patient()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient("Regression", "Unsuspend_patient", "M"); 
...
function tc_patient_change_the_patients_registered_practice() 
{
  try
  {
    ...
    login(5, "Shared");
    add_patient("Regression", "Registered_practice", "M"); 
...
function tc_transfer_a_patient_who_has_a_pending_treatment()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient('Regression', 'Transfer_pending', 'M'); 
    add_treatment_plan('W','Manual','','Shared','');
...
function tc_transfer_a_patient_where_the_patient_will_be_a_duplicate_of_an_existing_inactive_patient_at_the_destination_location()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient('Regression', 'Transfer_inactive_patient', 'M');
    ...
    login(15, "Shared");
    add_patient('Regression', 'Trans_inactive_pat', 'M', nhs_num_one); 
    ...
    var result_set_1 = accept_patient_in_transfer_request_message(messagename);
    result_set.push(result_set_1);
...
function tc_transfer_a_patient_who_is_on_an_induction_protocol()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient('Regression', 'Transfer_induction_patient', 'M');
    var messagename = get_patient_fullname();
...
function tc_reactivate_a_potential_duplicate_patient()
{
  try
  {
    ...
    add_patient('Regression', unique_val, 'M');
...
function tc_suspending_an_overdue_patient_removes_them_from_the_overdue_report()
{
  try
  {
    ...
    login(5, "Shared");
    add_patient("Regression", "overdue_suspend_patient", "M"
    ); 
