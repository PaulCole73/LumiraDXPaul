...
function convert_date_format(date, input_type, output_format)
{
  ...
  if(language == "Italian")
  {
    switch(output_format)  //examples are only for format, these format outputs will appear in italian
  ...
  {
    format_date = aqConvert.DateTimeToFormatStr(date_as_dd_mm_yyyy, output_format);
  }
   
  return format_date;
}
//-----------------------------------------------------------------------------------
//This will generate a random text string by combining strings it is used by the under the hood functions
