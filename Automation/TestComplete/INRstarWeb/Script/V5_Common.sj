...
//       INRstarV5 = p1.Page("http://scsl.inrstar.test/");
//    }
...
//------------------------------------------------------------------------
//Getting nhs from recently viewed
...
//------------------------------------------------------------------------
//Getting nhs from the patient banner
...
//----------------------------------------------------------------------------------------------
// Checks the top audit record action matches what was passed in and returns the data
...
//  WaitSeconds(2)
   ...
   var wt_row = wt_audit.Cell(1, 1).innerText;
    ...
    if (wt_row == w_data)
       Log.Message("audit_action matches");
     else
        {
       Log.Warning("Patient audit record not found");
        }
...
// Returns more information in the top audit record

function more_info_top_audit(w_data)
{  
   var INRstarV5 = set_system();
   var panelMCP = INRstarV5.Panel("MainPage").Panel("main").Panel("MainContentPanel")
   var panelPMTC = panelMCP.Panel("PatientRecord").Panel("PatientMainTabContent");
   var wt_audit = panelPMTC.Panel("PatientAuditTrailWrapper").Table("AuditTrailTable");
   var more_info = wt_audit.Cell(1, 3).innerText;
